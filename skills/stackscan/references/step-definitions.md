...
- **Condition:** SKIP this step only if `toolFitVerdict` is `"GOOD"` (all tools rated good). If `toolFitVerdict` is `"ADEQUATE"` or `"POOR"`, run this step. If skipped, record in progress.json as `{ "name": "discover_tools", "status": "skipped", "reason": "toolFitVerdict is GOOD" }` and move on.
- **Task:** Discover alternative investment opportunities (tools) that could address gaps in ADEQUATE or POOR-rated tools. For ADEQUATE tools, focus on targeted comparison: find as many quality alternatives as needed to make a sound recommendation. For POOR tools, run full discovery across the relevant pain categories. Use WebSearch/WebFetch if available. Present findings in a table with columns: Tool Name, URL, Pricing, Key Features, Addresses Gap. Update `targetTools` shared fact with any newly discovered tool names.
...
- **Task:** Create a phased implementation roadmap with payback milestones. Include: phase timeline, investments per phase, responsible roles, success criteria, rollback triggers, and when each investment starts paying back. Apply Team Size Scaling Rules. For very small teams (3 or fewer), combine phases and keep it practical: no pilot groups, no change champions, no formal training sessions.
  - All phase durations must be expressed in calendar weeks, not work-hours.
  - Each phase must state the estimated hours required (e.g., "Phase 1: 2 weeks, approx. 4 hrs setup").
  - Do not generate a timeline that implies more capacity than is realistic given the company profile. If the user is a solo operator or part-time, flag any phase that exceeds what one person can reasonably do in a week alongside their normal work, and note it as a risk. The user confirms or adjusts this at Gate 2.
...
- **Task:** Generate a ranked portfolio of operational investment opportunities ordered by payback period. Group by priority (immediate quick wins, short-term investments, medium-term bets). Each investment: what, who, when, dependencies, cost, expected return, and payback period. Include quick wins (< 1 hour to implement, immediate payoff) at the top. Reference specific tools, costs, and time savings from prior outputs. Numbers must match ROI.md exactly.
  For each investment item, apply the two-track scoring formula from SKILL.md: label the track (Process/Tool or Product), show the score breakdown per component (all four components with their sub-scores), and flag any component value that was estimated rather than measured.
